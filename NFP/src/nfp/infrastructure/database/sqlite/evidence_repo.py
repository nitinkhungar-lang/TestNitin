...
class SQLiteEvidenceRepository(EvidenceRepository):
    """SQLite repository for Evidence."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: Evidence) -> None:
        fields = [f.name for f in dataclasses.fields(Evidence) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO evidence (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO evidence ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[Evidence]:
        fields = [f.name for f in dataclasses.fields(Evidence) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM evidence WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(Evidence)
        kwargs = {}
    ...
    def get_all(self) -> List[Evidence]:
        cursor = self.conn.execute(f"SELECT * FROM evidence")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(Evidence):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(Evidence(**kwargs))
        return results

    def get_by_hash(self, file_hash: str) -> Optional[Evidence]:
        fields = [f.name for f in dataclasses.fields(Evidence) if not f.name.startswith('_')]
        if "file_hash" not in fields:
            return None
            
        cursor = self.conn.execute("SELECT * FROM evidence WHERE file_hash = ?", (file_hash,))
        row = cursor.fetchone()
        if not row:
            return None
