"""SQLite implementation of Ledger repository."""
import sqlite3
import json
import uuid
...
from nfp.repositories.ledger_repo import LedgerRepository
...
class SQLiteLedgerRepository(LedgerRepository):
    """SQLite repository for Ledger."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: Ledger) -> None:
        fields = [f.name for f in dataclasses.fields(Ledger) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO ledger (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO ledger ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[Ledger]:
        fields = [f.name for f in dataclasses.fields(Ledger) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM ledger WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(Ledger)
        kwargs = {}
    ...
    def get_all(self) -> List[Ledger]:
        cursor = self.conn.execute(f"SELECT * FROM ledger")
