"""SQLite implementation of ExchangeRate repository."""
import sqlite3
import json
import uuid
...
from nfp.repositories.exchange_rate_repo import ExchangeRateRepository
...
class SQLiteExchangeRateRepository(ExchangeRateRepository):
    """SQLite repository for ExchangeRate."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: ExchangeRate) -> None:
        fields = [f.name for f in dataclasses.fields(ExchangeRate) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO exchange_rate (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO exchange_rate ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[ExchangeRate]:
        fields = [f.name for f in dataclasses.fields(ExchangeRate) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM exchange_rate WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(ExchangeRate)
        kwargs = {}
        for field in dataclasses.fields(ExchangeRate):
            if field.name.startswith('_'): continue
            val = row[field.name]
            kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            
        return ExchangeRate(**kwargs)
        
    def get_all(self) -> List[ExchangeRate]:
        cursor = self.conn.execute(f"SELECT * FROM exchange_rate")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(ExchangeRate):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(ExchangeRate(**kwargs))
        return results
