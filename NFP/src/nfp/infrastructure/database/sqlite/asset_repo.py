"""SQLite implementation of Asset repository."""
import sqlite3
import json
import uuid
...
class SQLiteAssetRepository(AssetRepository):
    """SQLite repository for Asset."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: Asset) -> None:
        fields = [f.name for f in dataclasses.fields(Asset) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO asset (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO asset ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[Asset]:
        ...
        cursor = self.conn.execute(f"SELECT * FROM asset WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(Asset)
        kwargs = {}
    ...
    def get_all(self) -> List[Asset]:
        cursor = self.conn.execute(f"SELECT * FROM asset")
