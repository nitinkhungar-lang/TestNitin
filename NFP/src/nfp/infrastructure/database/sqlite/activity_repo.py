"""SQLite implementation of BusinessActivity repository."""
import sqlite3
import json
import uuid
...
class SQLiteBusinessActivityRepository(BusinessActivityRepository):
    """SQLite repository for BusinessActivity."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: BusinessActivity) -> None:
        fields = [f.name for f in dataclasses.fields(BusinessActivity) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO business_activity (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO business_activity ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[BusinessActivity]:
        fields = [f.name for f in dataclasses.fields(BusinessActivity) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM business_activity WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(BusinessActivity)
        kwargs = {}
        for field in dataclasses.fields(BusinessActivity):
            if field.name.startswith('_'): continue
            val = row[field.name]
            kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            
        return BusinessActivity(**kwargs)
        
    def get_all(self) -> List[BusinessActivity]:
        cursor = self.conn.execute(f"SELECT * FROM business_activity")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(BusinessActivity):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(BusinessActivity(**kwargs))
        return results
