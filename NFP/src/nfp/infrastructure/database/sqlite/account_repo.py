"""SQLite implementation of FinancialAccount repository."""
import sqlite3
import json
import uuid
...
class SQLiteFinancialAccountRepository(FinancialAccountRepository):
    """SQLite repository for FinancialAccount."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: FinancialAccount) -> None:
        fields = [f.name for f in dataclasses.fields(FinancialAccount) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO financial_account (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO financial_account ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[FinancialAccount]:
        fields = [f.name for f in dataclasses.fields(FinancialAccount) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM financial_account WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(FinancialAccount)
        kwargs = {}
        for field in dataclasses.fields(FinancialAccount):
            if field.name.startswith('_'): continue
            val = row[field.name]
            kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            
        return FinancialAccount(**kwargs)
        
    def get_all(self) -> List[FinancialAccount]:
        cursor = self.conn.execute(f"SELECT * FROM financial_account")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(FinancialAccount):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(FinancialAccount(**kwargs))
        return results
