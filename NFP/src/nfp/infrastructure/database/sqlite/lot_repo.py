...
class SQLiteLotRepository(LotRepository):
    """SQLite repository for Lot."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: Lot) -> None:
        fields = [f.name for f in dataclasses.fields(Lot) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO lot (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO lot ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[Lot]:
        fields = [f.name for f in dataclasses.fields(Lot) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM lot WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(Lot)
        kwargs = {}
        for field in dataclasses.fields(Lot):
    ...
    def get_all(self) -> List[Lot]:
        cursor = self.conn.execute(f"SELECT * FROM lot")
