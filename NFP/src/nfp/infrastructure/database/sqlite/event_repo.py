"""SQLite implementation of FinancialEvent repository."""
import sqlite3
import json
import uuid
...
class SQLiteFinancialEventRepository(FinancialEventRepository):
    """SQLite repository for FinancialEvent."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: FinancialEvent) -> None:
        fields = [f.name for f in dataclasses.fields(FinancialEvent) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO financial_event (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO financial_event ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[FinancialEvent]:
        fields = [f.name for f in dataclasses.fields(FinancialEvent) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM financial_event WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(FinancialEvent)
        kwargs = {}
        for field in dataclasses.fields(FinancialEvent):
    ...
    def get_all(self) -> List[FinancialEvent]:
        cursor = self.conn.execute(f"SELECT * FROM financial_event")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(FinancialEvent):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(FinancialEvent(**kwargs))
        return results
