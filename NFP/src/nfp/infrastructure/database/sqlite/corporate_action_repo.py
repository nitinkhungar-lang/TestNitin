"""SQLite implementation of CorporateAction repository."""
import sqlite3
import json
import uuid
...
def from_db(value: Any, field_type: Any) -> Any:
    ...
    if "bool" in type_str:
        return bool(value)
    if "SplitRatio" in type_str:
        d = json.loads(value)
        return SplitRatio(
            old_face_value=Decimal(str(d["old_face_value"])),
            new_face_value=Decimal(str(d["new_face_value"])),
            numerator=int(d.get("ratio_numerator", d.get("numerator", 1))),
            denominator=int(d.get("ratio_denominator", d.get("denominator", 1)))
        )
...
class SQLiteCorporateActionRepository(CorporateActionRepository):
    """SQLite repository for CorporateAction."""
    def __init__(self, conn: sqlite3.Connection):
        self.conn = conn

    def add(self, entity: CorporateAction) -> None:
        fields = [f.name for f in dataclasses.fields(CorporateAction) if not f.name.startswith('_')]
        cols = ", ".join(fields)
        ...
        if "id" not in fields:
            ...
            try:
                self.conn.execute(
                    f"INSERT OR REPLACE INTO corporate_action (id, {cols}) VALUES (?, {placeholders})",
                    [str(uuid.uuid4())] + values
                )
            except sqlite3.OperationalError:
                # No id column
                self.conn.execute(
                    f"INSERT OR REPLACE INTO corporate_action ({cols}) VALUES ({placeholders})",
                    values
                )
        else:
            self.conn.execute(
    ...
    def get(self, id: UUID) -> Optional[CorporateAction]:
        fields = [f.name for f in dataclasses.fields(CorporateAction) if not f.name.startswith('_')]
        if "id" not in fields:
            return None # Cannot get by ID
            
        cursor = self.conn.execute(f"SELECT * FROM corporate_action WHERE id = ?", (str(id),))
        row = cursor.fetchone()
        if not row:
            return None
            
        hints = get_type_hints(CorporateAction)
        kwargs = {}
        for field in dataclasses.fields(CorporateAction):
    ...
    def get_all(self) -> List[CorporateAction]:
        cursor = self.conn.execute(f"SELECT * FROM corporate_action")
        ...
        for row in rows:
            ...
            for field in dataclasses.fields(CorporateAction):
                ...
                val = row[field.name]
                kwargs[field.name] = from_db(val, hints.get(field.name, field.type))
            results.append(CorporateAction(**kwargs))
        return results
